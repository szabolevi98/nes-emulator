...
public sealed class Nes
{
    ...
    private const uint StateMagic = 0x53454E0C; // "NES" and a format version
    ...
    public void LoadState(Stream stream)
    {
        ...
        if (magic != StateMagic && magic != PortStateMagic && magic != AbStateMagic && magic != StopStateMagic && magic != SpriteStateMagic && magic != M2StateMagic && magic != IrqStateMagic && magic != DmcStateMagic && magic != ApuStateMagic && magic != NmiStateMagic && !legacy)
        {
            throw new InvalidDataException("Unsupported save state format. A v2 through v12 state is required.");
        }
        ...
        bool legacyBusDecay = magic < StateMagic;
        ...
        if (length != current.Length - (legacy ? 2 : 0) - (legacyApu ? 4 : 0) - (legacyDmc ? 7 : 0)
            ...
            - (legacyPort ? 3 : 0) - (legacyBusDecay ? 65 : 0))
        ...
        ReadStatePayload(new BinaryReader(new MemoryStream(data)), legacy, legacyApu, legacyDmc, legacyFilter, legacySprites, legacyStop, legacyPort, legacyBusDecay);
    }

    private Mmc3IrqRevision StateIrqRevision => Mapper is Mmc3 mmc3
        ? mmc3.IrqRevision : Mmc3IrqRevision.Standard;

    private void ReadStatePayload(BinaryReader reader, bool legacy, bool legacyApu, bool legacyDmc, bool legacyFilter, bool legacySprites, bool legacyStop, bool legacyPort, bool legacyBusDecay)
    {
        Cpu.LoadState(reader, legacy);
        bool legacyPpuNmiPending = Ppu.LoadState(reader, legacy, legacySprites, legacyBusDecay);
