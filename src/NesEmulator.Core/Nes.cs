...
public sealed class Nes
{
    ...
    private const uint StateMagic = 0x53454E0D; // "NES" and a format version
    ...
    public void LoadState(Stream stream)
    {
        ...
        if (magic != StateMagic && magic != BusStateMagic && magic != PortStateMagic && magic != AbStateMagic && magic != StopStateMagic && magic != SpriteStateMagic && magic != M2StateMagic && magic != IrqStateMagic && magic != DmcStateMagic && magic != ApuStateMagic && magic != NmiStateMagic && !legacy)
        {
            throw new InvalidDataException("Unsupported save state format. A v2 through v13 state is required.");
        }
        ...
        bool legacyEmphasis = magic < StateMagic;
        ...
        ReadStatePayload(new BinaryReader(new MemoryStream(data)), legacy, legacyApu, legacyDmc, legacyFilter, legacySprites, legacyStop, legacyPort, legacyBusDecay, legacyEmphasis);
    }

    private Mmc3IrqRevision StateIrqRevision => Mapper is Mmc3 mmc3
        ? mmc3.IrqRevision : Mmc3IrqRevision.Standard;

    private void ReadStatePayload(BinaryReader reader, bool legacy, bool legacyApu, bool legacyDmc, bool legacyFilter, bool legacySprites, bool legacyStop, bool legacyPort, bool legacyBusDecay, bool legacyEmphasis)
    {
