...
Run `dotnet run -c Release --project tests/NesEmulator.Tests` for the 263 offline checks. Audio regressions cover all 16 noise periods against a CPU-clocked reference sequence, DMC bit spacing and continuous looping, stopping a loop, and the following at both 44.1 and 48 kHz:
...
The public `8-dmc_rates` ROM now passes; with the subsequent PPU/NMI timing changes, the full public suite passes 44/55. All previously passing ROMs remain passing. The complete results, including failures, are in [accuracy-results.md](accuracy-results.md).
