...
- **Picture unit** — background and sprites, scrolling, palettes, sprite zero hit, per-dot sprite evaluation with the overflow bug, edge clipping, drawn one pixel per cycle as the beam sweeps
...
- **Cartridges** — iNES parsing and mappers 0, 1, 2, 3 and 4, which between them cover most of the library; the MMC3 filters A12 using actual CPU M2 edges, with selectable standard or alternate MMC3A IRQ behavior, and a selectable `$EE`/`$FF` model for the unstable `$AB` opcode
...
| **AccuracyCoin, complete collection** | **118 / 144** | Every test in the ROM, run individually; 5 informational entries are not counted |
...
```
dotnet run --project tests/NesEmulator.Tests      # 717 offline checks
