...
| **Public ROM total** | **54 / 55** | MMC3 revision selected to match each test's hardware target |
...
The remaining baseline failure is a deliberate disagreement: for the unstable `$AB` opcode this emulator follows the `$EE` mask used by the [SingleStepTests vectors](https://github.com/SingleStepTests/65x02/tree/2f6980a2d95757486c7bee24355c360e40e2a224/nes6502), while blargg's ROM assumes a different result. The discrepancy is reported rather than hidden by skipping the opcode. DMA stop/abort cases, OAM corruption and rendering-toggle quirks still need work. The separate [DMA report](docs/dma-results.md) and [sprite report](docs/sprite-results.md) keep the original 55-ROM denominator stable.
...
```
dotnet run --project tests/NesEmulator.Tests      # 641 offline checks
